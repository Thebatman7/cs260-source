...
const userSchema = new mongoose.Schema({
    ...
    username: String,
    password: String,
    role: { //this was added to have administrators and users
        type: String,
        default: ""
    }
});
...
This middleware will receive an incoming request from a browser and check whether req.session.userID exists.
If not, the user is not logged in, so it returns a 403 error.
Then it uses this userID to find the user's document in the database.
If that doesn't exist, it also returns a 403 errors. Otherwise, it sets req.user so that any endpoint
can use this to get the current logged in user, then calls next(), which passes control to the next middleware.
...
router.post('/', async (req, res) => {
    ...
    try {
      ...
      await user.save();
      // set user session info. using the middleware. This is necessary to keep user logged in
...
//After you added middleware and adding code to keep user session
// get logged in user
...
module.exports = {
    routes: router,
    model: User,
    valid: validUser //we modify this so we can use the middleware to keep user logged in
};
