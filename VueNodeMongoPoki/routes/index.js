...
MongoClient.connect(dbUrl, { useUnifiedTopology: true, useNewUrlParser: true }, function(err, client) {
  if (err) {
    ...
    // We insert data here, once we've successfully connected
    ...
    db.createCollection('poke', function(err, result) {
      ...
      collection.stats(function(err, stats) {
        ...
        if (stats.count == 0) { // If we haven't inserted before, put the default in
...
/* Call to query the database */
router.get('/pokemon', function(req, res) {
  console.log("In Pokemon");
  collection.find().toArray(function(err, result) { //.find gets all the data
    if(err) { console.log(err); }
...
/*
This is what we had before when we had the databse in same local machine
